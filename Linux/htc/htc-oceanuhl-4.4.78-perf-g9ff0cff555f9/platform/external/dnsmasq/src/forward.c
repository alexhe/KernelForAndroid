...
void receive_query(struct listener *listen, time_t now)
{
  ...
  // ++ PCND0009_DNSMASQ_DISCARD_DNS_QUERY_IF_NO_RECURSION_BIT
  /* [start] HTC Network - 20131017 , DNS cache snooping security issue. */
  if (header->rd == 0)
    {
      my_syslog(LOG_INFO, _("Discard DNS query that do not have recursion bit set"));
      return;
    }
  /* [end] */
  // -- PCND0009_DNSMASQ_DISCARD_DNS_QUERY_IF_NO_RECURSION_BIT
