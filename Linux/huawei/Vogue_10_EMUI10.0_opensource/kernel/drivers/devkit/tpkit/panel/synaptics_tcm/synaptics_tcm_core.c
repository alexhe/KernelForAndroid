...
static void syna_tcm_parse_basic_dts(struct device_node *np, struct ts_kit_device_data *chip_data)
{
	...
	retval = of_property_read_u32(np, "use_old_projectid", &bdata->use_old_projectid);
	if (retval < 0) {
		TS_LOG_ERR("get device use_old_projectid failed\n");
		bdata->use_old_projectid = 0;
	}
