...
{
    int ret = 0;
    bool fSnclk2 = (index == 2 || index == 3 || index == 4); // 2 3 4 sensor index for different clk_isp_snclk
	...
	cam_info("%s enter.index(%u), clk(%u), on(%d)", __func__, index, clk, on);
	...
	if(on) {
		...
		} else if (index == 1 || index == 6) { // 1 6 sensor index for different clk_isp_snclk
        ...
        if ((index == 0) && (s_ctrl->isp_snclk0 != NULL)) {
            clk_disable_unprepare(s_ctrl->isp_snclk0);
            cam_debug("clk_disable_unprepare snclk0.\n");
        } else if (((index == 1) || (index == 6)) && (s_ctrl->isp_snclk1 != NULL)) { // 1 6 sensor index for different clk_isp_snclk
	...
	struct sensor_power_setting *power_setting, int state)
{
	...
	if(pin_type == FSIN) {
		...
		rc = gpio_direction_output(sensor_info->gpios[pin_type].gpio,
			state ? (power_setting->config_val + 1) % 2 : power_setting->config_val); // 2 for Val Calculation
		if(rc < 0) {
...
int load_power_config_switch_1(struct sensor_power_setting *powerSetting, sensor_t *sCtrl, int onOff, int *rc)
{
	if (powerSetting == NULL || sCtrl == NULL)
		return -1;
	switch (powerSetting->seq_type) {
	case SENSOR_DVDD:
		cam_debug("%s, seq_type:%u SENSOR_DVDD", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_DVDD, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_DVDD2:
		cam_debug("%s, seq_type:%u SENSOR_DVDD2", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_DVDD2, sCtrl->board_info, powerSetting, onOff);
		if (*rc) {
		...
		break;
	case SENSOR_OIS_DRV:
		cam_debug("%s, seq_type:%u SENSOR_OIS_DRV", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_OISDRV, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_IOVDD:
		cam_debug("%s, seq_type:%u SENSOR_IOVDD", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_IOVDD, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_AVDD:
		cam_debug("%s, seq_type:%u SENSOR_AVDD", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_AVDD, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_AVDD2:
		cam_debug("%s, seq_type:%u SENSOR_AVDD2", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_AVDD2, sCtrl->board_info, powerSetting, onOff);
		if (*rc) {
			cam_err("%s power up procedure error.", __func__);
			*rc = 0;
		}
	...
	case SENSOR_VCM_AVDD:
		cam_debug("%s, seq_type:%u SENSOR_VCM_AVDD", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_VCM, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_VCM_AVDD2:
		cam_debug("%s, seq_type:%u SENSOR_VCM_AVDD2", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_VCM2, sCtrl->board_info, powerSetting, onOff);
		if (*rc) {
			cam_err("%s power up procedure error.", __func__);
			*rc = 0;
		}
		break;
	case SENSOR_AVDD0:
		cam_debug("%s, seq_type:%u SENSOR_AVDD0", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_AVDD0, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_AVDD1:
		cam_debug("%s, seq_type:%u SENSOR_AVDD1", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_AVDD1, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_MISP_VDD:
		cam_debug("%s, seq_type:%u SENSOR_MISP_VDD", __func__, powerSetting->seq_type);
		*rc = hw_sensor_ldo_config(LDO_MISP, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_MCLK:
		cam_debug("%s, seq_type:%u SENSOR_MCLK", __func__, powerSetting->seq_type);
		*rc = hw_mclk_config(sCtrl, powerSetting, onOff);
		break;
	case SENSOR_I2C:
		if (onOff == POWER_ON) {
			cam_debug("%s, seq_type:%u SENSOR_I2C", __func__, powerSetting->seq_type);
			hw_sensor_i2c_config(sCtrl, powerSetting, POWER_ON);
		}
		break;
	case SENSOR_LDO_EN:
		cam_debug("%s, seq_type:%u SENSOR_LDO_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(LDO_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_AVDD1_EN:
		cam_debug("%s, seq_type:%u SENSOR_AVDD1_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(AVDD1_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_DVDD0_EN:
		cam_debug("%s, seq_type:%u SENSOR_DVDD0_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(DVDD0_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_DVDD1_EN:
		cam_debug("%s, seq_type:%u SENSOR_DVDD1_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(DVDD1_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_IOVDD_EN:
		cam_debug("%s, seq_type:%u SENSOR_IOVDD_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(IOVDD_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_MISPDCDC_EN:
		cam_debug("%s, seq_type:%u SENSOR_MISPDCDC_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(MISPDCDC_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	default:
		cam_err("%s invalid seq_type (%d).", __func__, powerSetting->seq_type);
		return -1;
	}
	return 0;
}

void load_power_config_switch_2(struct sensor_power_setting *powerSetting, sensor_t *sCtrl, int onOff, int *rc)
{
	struct hw_comm_pmic_cfg_t fpPmicldoSet = {0};
	if (powerSetting == NULL || sCtrl == NULL)
		return;
	...
	case SENSOR_CHECK_LEVEL:
		if (onOff == POWER_ON) {
			cam_debug("%s, seq_type:%u SENSOR_CHECK_LEVEL", __func__, powerSetting->seq_type);
			*rc = hw_sensor_gpio_config(FSIN, sCtrl->board_info, powerSetting, POWER_ON);
		}
		break;
	case SENSOR_RST:
		cam_debug("%s, seq_type:%u SENSOR_RST", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(RESETB, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_PWDN:
		cam_debug("%s, seq_type:%u SENSOR_PWDN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(PWDN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_VCM_PWDN:
		cam_debug("%s, seq_type:%u SENSOR_VCM_PWDN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(VCM, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_SUSPEND:
		cam_debug("%s, seq_type:%u SENSOR_SUSPEND", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(SUSPEND, sCtrl->board_info, powerSetting, POWER_OFF);
		break;
	case SENSOR_SUSPEND2:
		cam_debug("%s, seq_type:%u SENSOR_SUSPEND2", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(SUSPEND2, sCtrl->board_info, powerSetting, POWER_OFF);
		break;
	case SENSOR_RST2:
		cam_debug("%s, seq_type:%u SENSOR_RST2", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(RESETB2, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_OIS:
		cam_debug("%s, seq_type:%u SENSOR_OIS", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(OIS, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_OIS2:
		cam_debug("%s, seq_type:%u SENSOR_OIS2", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(OIS2, sCtrl->board_info, powerSetting, onOff);
		if (*rc) {
			cam_err("%s power up procedure error.", __func__);
			*rc = 0;
		}
		break;
	case SENSOR_PMIC:
		cam_debug("%s, seq_type:%u SENSOR_PMIC", __func__, powerSetting->seq_type);
		*rc = hw_sensor_pmic_config(sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_PMIC2:
		cam_info("%s, seq_type:%u SENSOR_PMIC2", __func__, powerSetting->seq_val);
		fpPmicldoSet.pmic_num = 1;
		fpPmicldoSet.pmic_power_type = powerSetting->seq_val;
		fpPmicldoSet.pmic_power_voltage = powerSetting->config_val;
		fpPmicldoSet.pmic_power_state = onOff;
		*rc = hw_pmic_power_cfg(MAIN_CAM_PMIC_REQ, &fpPmicldoSet);
		if (*rc) {
			cam_err("%s Maybe failed to hw_pmic_power_cfg, rc=%d", __func__, *rc);
			*rc = 0;
		};
		mdelay(1);
	...
	case SENSOR_AVDD2_EN:
		cam_debug("%s, seq_type:%u SENSOR_AVDD2_EN", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(AVDD2_EN, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_MIPI_SW:
		cam_info("%s, seq_type:%u SENSOR_MIPI_SW", __func__, powerSetting->seq_type);
		*rc = hw_sensor_gpio_config(MIPI_SW, sCtrl->board_info, powerSetting, onOff);
		break;
	case SENSOR_LASER_XSHUT:
		cam_info("%s, seq_type:%u SENSOR_LASER_XSHUT", __func__, powerSetting->seq_type);
		if (sCtrl->board_info->gpios[LASER_XSHUT].gpio == 0) {
			cam_err("gpio type[%d] is not actived", LASER_XSHUT);
			*rc = 0;
			break;
		}
		*rc = gpio_direction_output(sCtrl->board_info->gpios[LASER_XSHUT].gpio, onOff);
		if (*rc < 0) {
			cam_err("failed to control gpio[%d]", sCtrl->board_info->gpios[LASER_XSHUT].gpio);
		} else {
			cam_info("%s config gpio[%d] output[%d].", __func__, sCtrl->board_info->gpios[LASER_XSHUT].gpio, onOff);
		}
		break;
	default:
		cam_err("%s invalid seq_type (%d).", __func__, powerSetting->seq_type);
...
int hw_sensor_power_config(struct sensor_power_setting *powerSetting, sensor_t *sCtrl, int onOff)
{
    int rc = 0;
	int ret = 0;
	ret = load_power_config_switch_1(powerSetting, sCtrl, onOff, &rc);
	if (ret < 0)
		load_power_config_switch_2(powerSetting, sCtrl, onOff, &rc);
    return rc;
}
...
int hwsensor_writefile(int index, const char *sensor_name)
{
	...
	filp = filp_open(file_name, O_CREAT|O_WRONLY, 0666); // 0666 for filp_open mode set
...
int hw_sensor_get_ext_dt_data_count(struct device_node *of_node, hwsensor_board_info_t *sensor_info)
{
	const char* ext_name = NULL;
	unsigned int count = of_property_count_strings(of_node, "huawei,ext_name");
	unsigned int i = 0;
	int ret = 0;
	if (( count > 0) && (count <= EXT_NAME_NUM)) {
		for (i = 0; i < count; i++) {
			int rc = of_property_read_string_index(of_node, "huawei,ext_name", i, &ext_name);
			if (rc < 0) {
				sensor_info->ext_type = NO_EXT_INFO;
				cam_err("%s get ext_name failed %d\n", __func__, __LINE__);
				return -EINVAL;
			}
			ret = strncpy_s(sensor_info->ext_name[i], DEVICE_NAME_SIZE - 1, ext_name, strlen(ext_name) + 1);
			if (ret != 0) {
...
int hw_sensor_get_ext_dt_data(struct device_node *of_node, hwsensor_board_info_t *sensor_info)
{
	...
	if(sensor_info->ext_type == EXT_INFO_NO_ADC)
	{
		...
		if (rc < 0) {
			...
			ret = strncpy_s(sensor_info->ext_name[0], DEVICE_NAME_SIZE-1, ext_name, strlen(ext_name)+1);
			if (ret != 0) {
				cam_err("%s strncpy_s return error.\n", __func__);
			}
...
int get_dt_data_1(struct device_node *of_node, hwsensor_board_info_t *sensor_info, int *rc)
{
	int count = 0;
	...
	*rc = of_property_read_u32(of_node, "huawei,vcmpd_valid",
	...
	count = of_property_count_elems_of_size(of_node, "huawei,csi_index",
		sizeof(u32));
	if (count > 0) {
		of_property_read_u32_array(of_node, "huawei,csi_index",
	...
	count = of_property_count_elems_of_size(of_node, "huawei,i2c_index",
		sizeof(u32));
	if (count > 0) {
		of_property_read_u32_array(of_node, "huawei,i2c_index",
...
int get_dt_data_2(struct platform_device *pdev, struct device_node *of_node, hwsensor_board_info_t *sensor_info, int *rc)
{
	int i = 0;
	char *gpio_tag = NULL;
	int index = 0;
	...
	if (pdev == NULL || of_node == NULL || sensor_info == NULL)
        return -1;
...
int get_dt_data_3(struct device_node *of_node, hwsensor_board_info_t *sensor_info, int *rc)
{
    if (of_node == NULL || sensor_info == NULL)
        return -1;
	...
	*rc = of_property_read_u32(of_node, "huawei,topology_type", &sensor_info->topology_type);
	...
	*rc = of_property_read_u32(of_node, "huawei,phyinfo_valid",
	...
	if (!*rc && (1 == sensor_info->phyinfo_valid || 2 == sensor_info->phyinfo_valid)) { // 1 2 for phyinfo count
...
static void set_power_settings(int i, struct sensor_power_setting **power_settings, const char *seq_name,
	uint32_t *seq_vals, uint32_t *cfg_vals, uint32_t *sensor_indexs, uint32_t *seq_delays)
{
...
{
	const char *seq_val_name = (is_power_on ? "huawei,cam-power-seq-val" : "huawei,cam-power-down-seq-val");
...
static int kzalloc_cfg(int *rc, int count, int is_power_on,
	struct device_node *dev_node, uint32_t *cfg_vals, uint32_t **sensor_indexs)
{
	const char *seq_cfg_name = (is_power_on ? "huawei,cam-power-seq-cfg-val" : "huawei,cam-power-down-seq-cfg-val");
...
static int kzalloc_sensor(int *rc, int count, int is_power_on,
	struct device_node *dev_node, uint32_t *sensor_indexs, uint32_t **seq_delays)
{
	const char *seq_sensor_index_name = (is_power_on ?
		"huawei,cam-power-seq-sensor-index" : "huawei,cam-power-down-seq-sensor-index");
...
static int kzalloc_delay(int *rc, int count, int is_power_on,
	struct device_node *dev_node, uint32_t *seq_delays, struct sensor_power_setting **power_settings)
{
	const char *seq_delay_name = (is_power_on ? "huawei,cam-power-seq-delay" : "huawei,cam-power-down-seq-delay");
	...
	struct sensor_power_setting_array *power_setting_array, int is_power_on)
{
	...
	if (kzalloc_val(&rc, count, is_power_on, dev_node, seq_vals, &cfg_vals) == -1) {
		goto OUT;
	}

	if (kzalloc_cfg(&rc, count, is_power_on, dev_node, cfg_vals, &sensor_indexs) == -1) {
		goto OUT;
	}

	if (kzalloc_sensor(&rc, count, is_power_on, dev_node, sensor_indexs, &seq_delays) == -1) {
		goto OUT;
	}

	if (kzalloc_delay(&rc, count, is_power_on, dev_node, seq_delays, &power_settings) == -1) {
		goto OUT;
	}
	...
	for (i = 0; i < (int)count; i++) {
		...
		set_power_settings(i, &power_settings, seq_name, seq_vals, cfg_vals, sensor_indexs, seq_delays);
	}
