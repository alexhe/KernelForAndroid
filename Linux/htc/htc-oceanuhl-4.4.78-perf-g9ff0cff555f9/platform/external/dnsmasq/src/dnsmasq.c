...
int main (int argc, char **argv)
{
  ...
  for (i = 0; i < max_fd; i++) {
    if (i != STDOUT_FILENO && i != STDERR_FILENO && i != STDIN_FILENO) {
      //** [start] Network - 20170814 Fix dnsmasq logging problem
      int family, type;
      socklen_t optlen = sizeof(family);
      if (getsockopt(i, SOL_SOCKET, SO_DOMAIN, &family, &optlen) == 0 &&
          getsockopt(i, SOL_SOCKET, SO_TYPE, &type, &optlen) == 0) {
          if (family == AF_UNIX && type == SOCK_DGRAM) {
            my_syslog(LOG_INFO, _("[NET] Ignore fd %d"), i);
            continue;
          }
      }
      //** [end] Network - 20170814
    close(i);
    }
  }
