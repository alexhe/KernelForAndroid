	...
	struct uscp_device_info *di)
{
	...
	(void)power_dts_read_u32(np, "uscp_threshold_tusb",
		(u32 *)&di->uscp_threshold_tusb, DEFAULT_TUSB_THRESHOLD);

	if (power_dts_read_u32(np, "open_mosfet_temp",
		&di->open_mosfet_temp, 0))
		return -EINVAL;

	(void)power_dts_read_u32(np, "open_hiz_temp",
		(u32 *)&di->open_hiz_temp, di->open_mosfet_temp);
	(void)power_dts_read_u32(np, "dmd_hiz_enable",
		(u32 *)&di->dmd_hiz_enable, DMD_HIZ_DISABLE);

	if (power_dts_read_u32(np, "close_mosfet_temp",
		(u32 *)&di->close_mosfet_temp, 0))
		return -EINVAL;

	if (power_dts_read_u32(np, "interval_switch_temp",
		(u32 *)&di->interval_switch_temp, 0))
		return -EINVAL;
