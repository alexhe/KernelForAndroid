...
static struct kobject *g_power_dsm_kobj;
...
struct power_dsm_data_info g_power_dsm_data[] = {
...
static int __init power_dsm_init(void)
{
...
#if (defined(POWER_DSM_DEBUG_TEST) && defined(CONFIG_SYSFS))
	g_power_dsm_kobj = kobject_create_and_add("power_dsm", NULL);
	if (!g_power_dsm_kobj) {
	...
	ret = sysfs_create_group(g_power_dsm_kobj, &power_dsm_attr_group);
	if (ret) {
...
#if (defined(POWER_DSM_DEBUG_TEST) && defined(CONFIG_SYSFS))
fail_create_sysfs:
	kobject_del(g_power_dsm_kobj);
	g_power_dsm_kobj = NULL;
fail_create_kobj:
	return ret;
#endif /* POWER_DSM_DEBUG_TEST && CONFIG_SYSFS */
}
...
#if (defined(POWER_DSM_DEBUG_TEST) && defined(CONFIG_SYSFS))
	sysfs_remove_group(g_power_dsm_kobj, &power_dsm_attr_group);
	kobject_del(g_power_dsm_kobj);
	g_power_dsm_kobj = NULL;
#endif /* POWER_DSM_DEBUG_TEST && CONFIG_SYSFS */
