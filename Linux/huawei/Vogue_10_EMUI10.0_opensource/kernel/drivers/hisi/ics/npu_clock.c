...
int npu_clock_init(struct device *dev)
{
	...
	if (memset_s(npu_clock_manager, sizeof(npu_clock_manager), 0, sizeof(npu_clock_manager)) != EOK) {
		NPU_ERR("memset clock manager failed\n");
	}
	if (memset_s(&vcodecbus_clock_config, sizeof(vcodecbus_clock_config), 0, sizeof(vcodecbus_clock_config)) != EOK) {
		NPU_ERR("memset clock config failed\n");
	}
