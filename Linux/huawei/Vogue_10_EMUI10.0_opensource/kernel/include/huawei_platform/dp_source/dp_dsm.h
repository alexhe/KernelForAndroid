...
void dp_link_state_event(enum dp_link_state state);

#ifdef DP_DSM_ENABLE
void dp_imonitor_set_pd_event(uint8_t irq_type,
			      uint8_t cur_mode,
			      uint8_t mode_type,
			      uint8_t dev_type,
			      uint8_t typec_orien);
void dp_imonitor_set_param(enum dp_imonitor_param param, void *data);
void dp_imonitor_set_param_aux_rw(bool rw,
				  bool i2c,
				  uint32_t addr,
				  uint32_t len,
				  int retval);
void dp_imonitor_set_param_timing(uint16_t h_active,
				  uint16_t v_active,
...
static inline void dp_imonitor_set_param_aux_rw(bool rw,
						bool i2c,
						uint32_t addr,
						uint32_t len,
						int retval)
...
static inline void dp_imonitor_set_param_resolution(uint8_t *user_mode,
						    uint8_t *user_format)
{
}
...
#ifdef DP_DEBUG_ENABLE
static inline int dp_get_pd_event_result(char *buf, int size) { return 0; }
static inline int dp_get_hotplug_result(char *buf, int size) { return 0; }
static inline int dp_get_timing_info_result(char *buf, int size) { return 0; }
static inline int dp_get_vs_pe_result(char *buf, int size) { return 0; }
#endif // DP_DEBUG_ENABLE
#endif // DP_DSM_ENABLE
