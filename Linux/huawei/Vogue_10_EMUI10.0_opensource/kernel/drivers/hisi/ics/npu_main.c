...
{
    NPU_MAIN_PRIVATE_COMMON_S *common_dev = NULL;
    int coreid = 0;
...
static long npu_get_chip_type(unsigned long arg)
{
    ...
    if (memset_s(param, sizeof(param), 0x0, sizeof(param)) != EOK) {
...
static long npu_set_secure_mode(unsigned long arg)
{
    ...
    if (memset_s(&pm_runtime, sizeof(NPU_PM_RUNTIME_S), 0x0, sizeof(NPU_PM_RUNTIME_S)) != EOK) {
        NPU_ERR("memset_s failed\n");
    }
    if (0 == coreid) {
    ...
    CTRL_UNLOCK(coreid);  //lint !e455
    NPU_DEBUG(" npu_set_secure_mode coreid[%d]\n", coreid);
...
static long npu_set_reset_virt_addr(unsigned long arg)
{
    ...
    if (memset_s(&map_data, sizeof(struct map_data), 0x0, sizeof(struct map_data)) != EOK) {
        NPU_ERR("memset_s failed\n");
    }
...
static void npu_common_release_features(void)
{
    ...
    if (memset_s(&p_common->feature, sizeof(FEATURE_S), 0x0, sizeof(FEATURE_S)) != EOK) {
        NPU_ERR("memset_s failed\n");
    }
...
static void npu_core_reg_irq_exit(unsigned int coreid)
{
    ...
    if (memset_s(&p_core->npu_irq, sizeof(NPU_IRQ_REG_S), 0, sizeof(NPU_IRQ_REG_S)) != EOK) {
        NPU_ERR("memset_s failed\n");
    }
...
static int npu_core_reg_irq_init(unsigned int coreid, struct device *dev)
{
    ...
    NPU_MAIN_PRIVATE_CORE_S* p_core = NULL;
...
static int npu_core_init(unsigned int coreid, struct platform_device *pdev)
{
    ...
    wakeup_source_init(&p_core->wakelock, "npu_process");
...
static void npu_core_exit(unsigned int coreid)
{
    ...
    wakeup_source_trash(&p_core->wakelock);
...
static int npu_pm_runtime_suspend(struct device *dev)
{
    ...
    __pm_relax(&adapter->core[coreid].wakelock); //lint !e454  !e455
...
static int npu_pm_runtime_resume(struct device *dev)
{
    ...
    __pm_stay_awake(&adapter->core[coreid].wakelock); //lint !e454  !e455
    ret = npu_power_on(coreid);
    if (ret) {
...
static int npu_probe(struct platform_device *pdev)
{
    ...
    if (memset_s(&callback, sizeof(callback), 0x0, sizeof(callback)) != EOK) {
        NPU_ERR("memset_s failed\n");
    }
