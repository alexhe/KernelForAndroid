...
#define EXT2_DFL_CHECKINTERVAL (86400L * 180L)
	...
	/* For SEC feature */
	fs->super->s_def_resgid =
			get_int_from_profile(fs_types, "sec_def_resgid", 0);
	if (ext2fs_r_blocks_count(fs->super) == 0)
		ext2fs_r_blocks_count_set(fs->super,
			get_int_from_profile(fs_types, "sec_resv_blocks", 0));
	fs->super->s_sec_r_blocks_count =
			get_int_from_profile(fs_types, "sec_db_reserved", 0);
