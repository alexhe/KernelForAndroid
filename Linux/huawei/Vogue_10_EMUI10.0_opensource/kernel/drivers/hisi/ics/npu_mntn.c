...
static int npu_mntn_copy_reg_to_bbox(char *src_addr, unsigned int len)
{
	...
	if (memcpy_s(((char*)g_mntn_private->mntn_info.rdr_addr + g_mntn_private->mntn_info.bbox_addr_offset),
		NPU_BBOX_MEM_MAX - g_mntn_private->mntn_info.bbox_addr_offset,
		src_addr, len) != EOK) {
...
static void npu_mntn_write_peri_reg_info(unsigned int core_id)
{
	...
	int ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX, NPU_BUF_LEN_MAX-1,"peri_stat=%x, ppll_select=%x, power_stat=%x, power_ack=%x, reset_stat=%x, perclken=%x, perstat=%x.\r\n",
			 peri_reg->peri_stat,
...
static void npu_mntn_write_mstr_reg_info(unsigned int core_id)
{
	...
	int ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX, NPU_BUF_LEN_MAX-1, "RD_BITMAP=%x, WR_BITMAP=%x, rd_cmd_total_cnt[0-3]={%x, %x, %x}, wr_cmd_total_cnt=%x\n",
			mstr_reg->rd_bitmap,
			mstr_reg->wr_bitmap,
...
static void npu_mntn_write_reg_log(unsigned int core_id)
{
    ...
    if (memset_s((char*)g_mntn_private->mntn_info.rdr_addr, NPU_BBOX_MEM_MAX, 0, NPU_BBOX_MEM_MAX) != EOK) {
		NPU_ERR("memset_s failed\n");
    }

    ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX, NPU_BUF_LEN_MAX-1, "\r\ncore_id=0x%x\r\n", core_id);
	if (ret < 0) {
		NPU_ERR("snprintf_s failed\n");
		return;
	}
	...
	switch (g_mntn_private->mntn_info.dump_info.modid) {
		case MODID_NPU_EXC_DEAD:  //lint !e650
			ret = snprintf_s(log_buf, NPU_BUF_LEN_MAX, NPU_BUF_LEN_MAX-1, "interrupt_status=0x%x\r\n", g_mntn_private->exc_info[core_id].interrupt_status);
	        if (ret < 0) {
				NPU_ERR("snprintf_s failed\n");
				return;
			}
...
void npu_mntn_peri_reg_init(unsigned int core_id, struct npu_peri_reg_s *peri_reg_ptr)
{
	...
	if (memcpy_s(&g_mntn_private->reg_info[core_id].peri_reg, sizeof(g_mntn_private->reg_info[core_id].peri_reg), peri_reg_ptr, sizeof(struct npu_peri_reg_s)) != EOK) {
		NPU_ERR("memcpy_s failed\n");
	}
	return;
}
...
void npu_mntn_mstr_reg_init(unsigned int core_id, struct npu_mstr_reg_s *mstr_reg_ptr)
{
	...
	if (memcpy_s(&g_mntn_private->reg_info[core_id].mstr_reg, sizeof(g_mntn_private->reg_info[core_id].mstr_reg), mstr_reg_ptr, sizeof(struct npu_mstr_reg_s))!= EOK) {
		NPU_ERR("memcpy_s failed\n");
	}
	return;
}
...
static void npu_mntn_exc_info_init(unsigned int core_id, struct npu_exc_info_s *exc_info_ptr)
{
	...
	g_mntn_private->core_id = core_id;
	if (memcpy_s(&g_mntn_private->exc_info[core_id], sizeof(g_mntn_private->exc_info[core_id]), exc_info_ptr, sizeof(struct npu_exc_info_s)) !=EOK) {
		NPU_ERR("memcpy_s failed\n");
	}
...
void npu_mntn_power_exc_proc(unsigned int core_id, unsigned int target_ip, int result)
{
	...
	if (memset_s(&exc_info, sizeof(struct npu_exc_info_s), 0, sizeof(struct npu_exc_info_s))!= EOK) {
		NPU_ERR("memset_s failed\n");
	}
