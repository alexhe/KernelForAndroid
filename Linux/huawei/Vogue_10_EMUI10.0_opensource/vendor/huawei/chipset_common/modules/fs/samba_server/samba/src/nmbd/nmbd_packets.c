...
{
	LOGD("create_listen_fdset start");
	int *sock_array = NULL;
	struct subnet_record *subrec = NULL;
	int count = 0;
	int num = 0;
	...
	if(pset == NULL) {
		LOGD("create_listen_fdset: malloc fail!");
...
only use %d.\n", (count*2) + 2, FD_SETSIZE));
		LOGD("create_listen_fdset: Too many file descriptors needed (%d).We can only use %d", (count*2) + 2, FD_SETSIZE);
		SAFE_FREE(pset);
		return True;
	}
	...
	*psock_array = sock_array;
	...
	LOGD("create_listen_fdset finish ok");
	return False;
}
...
BOOL listen_for_packets(BOOL run_election)
{
	...
	if(listen_set == NULL || rescan_listen_set) {
		if(create_listen_fdset(&listen_set, &sock_array, &listen_number, &maxfd)) {
			LOGD("listen_for_packets : Fatal error. unable to create listen set. Exiting.");
    ...
    if (dns_fd != -1 && dns_fd != 0 && dns_fd < FD_SETSIZE) {
#else
