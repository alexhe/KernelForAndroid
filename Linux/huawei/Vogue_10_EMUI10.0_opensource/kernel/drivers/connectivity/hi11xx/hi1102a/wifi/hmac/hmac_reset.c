...
oal_uint32  hmac_config_query_chr_ext_info_rsp_event(mac_vap_stru *pst_mac_vap, oal_uint8 uc_len, oal_uint8 *puc_param)
{
    ...
    chr_wifi_ext_info_stru      *pst_query_chr_ext_info;
    oal_uint8                   uc_idx;

    pst_hmac_vap = mac_res_get_hmac_vap(pst_mac_vap->uc_vap_id);
    if (pst_hmac_vap == OAL_PTR_NULL) {
        OAM_ERROR_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_config_query_chr_ext_info_rsp_event::pst_hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
