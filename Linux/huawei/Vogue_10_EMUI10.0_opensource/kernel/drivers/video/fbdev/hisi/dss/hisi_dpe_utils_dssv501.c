...
void init_igm_gmp_xcc_gm(struct hisi_fb_data_type *hisifd)
{
	...
	if (pinfo->gmp_support == 1) {
		...
		HISI_FB_INFO("[effect] gmp enable=%d, lut_sel=%d, update_flag=%d, updated=%d, copy_status=%d\n", \
			lcp_param->gmp_enable, gmp_lut_sel, hisifd->effect_gmp_update_flag, hisifd->effect_updated_flag.gmp_effect_updated, hisifd->effect_gmp_copy_status);
		g_gmp_State = 1;
	} else {
		//gmp memory shutdown
