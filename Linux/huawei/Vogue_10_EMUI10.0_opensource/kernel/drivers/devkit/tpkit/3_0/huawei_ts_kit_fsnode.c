...
int ts_send_roi_cmd(enum ts_action_status read_write_type, int timeout)
{
	...
	if (pdata->feature_info.roi_info.roi_supported) {
		...
		cmd.cmd_param.prv_params = (void *)&pdata->feature_info.roi_info;
		if ((pdata->chip_data->is_direct_proc_cmd) &&
			(pdata->chip_data->is_can_device_use_int))
	/* Set this macro check to make sure bootup use the put one cmd */
			error = ts_kit_proc_command_directly(&cmd);
		else
				...
				const char *buf, size_t count)
{
	...
	TS_LOG_INFO("%s holster_supported = %d\n", __func__,
		g_ts_kit_platform_data.feature_info.holster_info.holster_supported);
				...
				struct device_attribute *attr, char *buf)
{
	...
	info = (struct ts_hand_info *)kzalloc(sizeof(struct ts_hand_info),
			GFP_KERNEL);
	if (!info) {
		TS_LOG_ERR("malloc failed\n");
		error = -ENOMEM;
				...
				size_t count)
{
	...
	info = (struct ts_regs_info *)kzalloc(sizeof(struct ts_regs_info),
			GFP_KERNEL);
	if (!info) {
		TS_LOG_ERR("malloc failed\n");
		error = -ENOMEM;
					...
					const char *buf, size_t size)
{
	...
	palm_info = (struct ts_palm_info *)kzalloc(sizeof(struct ts_palm_info),
		GFP_KERNEL);
	if (!palm_info) {
		TS_LOG_ERR("malloc failed\n");
		ret = -ENOMEM;
					...
					const char *buf, size_t count)
{
	...
	for (i = 0; i < str_num; i++) {
		...
		if (ret < 0)
			TS_LOG_ERR("anti_false_touch_get_param:return error,ret=%d\n", ret);
	}
	TS_LOG_INFO("%s -\n", __func__);
	return count;
}
				...
				const char *buf, size_t count)
{
	...
	if (info->roi_switch == value) {
		TS_LOG_INFO("%s, there is no need to send same cmd twice. roi_switch valie is %u",
			__func__, value);
		error = count;
		goto out;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	info = (struct ts_oem_info_param *)kzalloc(sizeof(struct ts_oem_info_param),
			GFP_KERNEL);
	if (!info) {
		TS_LOG_ERR("%s: malloc failed\n", __func__);
				...
				size_t count)
{
	...
	if (strlen(buf) > TS_GAMMA_DATA_MAX_SIZE - 1) {
		TS_LOG_ERR("%s: Store TPIC type size= %lu > MAX input size=%d\n",
...
{
	int error;
	struct ts_pen_info *info = &g_ts_kit_platform_data.feature_info.pen_info;
