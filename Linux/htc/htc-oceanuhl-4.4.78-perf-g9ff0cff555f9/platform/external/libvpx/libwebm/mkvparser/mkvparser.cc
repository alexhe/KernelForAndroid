...
Cues::Cues(Segment* pSegment, long long start_, long long size_,
    ...
    : m_pSegment(pSegment),
      ...
      m_preload_count(0),
      m_pos(start_) {
    ALOGD("CuesConstruct, cues info: start[%lld], size[%lld], pos[%lld]", m_start, m_size, m_pos);
}

Cues::~Cues() {
  ALOGD("CuesRelease, cues info: start[%lld], size[%lld], pos[%lld]", m_start, m_size, m_pos);
...
int Track::Info::Copy(Info& dst) const {
  ...
  if (codecPrivateSize > 0) {
    ...
    dst.codecPrivateOffset = codecPrivateOffset;
  }
...
const unsigned char* Track::GetCodecPrivate(size_t& size, long long& offset) const {
  size = m_info.codecPrivateSize;
  offset = m_info.codecPrivateOffset;
