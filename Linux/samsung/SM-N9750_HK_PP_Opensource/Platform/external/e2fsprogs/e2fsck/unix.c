...
static void stlog(e2fsck_t ctx, char const *fmt, ...)
{
	...
	if (fd < 0)
		log_out(ctx, _("Skip stlog(open failed(%s))\n"), strerror(errno));
	else {
		int ret = write(fd, buffer, len);
		if (ret < 0)
			log_out(ctx, _("Skip stlog(write failed(%s))\n"), strerror(errno));
		close(fd);
	}
}
...
static void check_if_skip(e2fsck_t ctx)
{
	...
	log_out(ctx, "\n");
skip:
	if (ext2fs_close_free(&ctx->fs)) {
		log_out(ctx,_("ext2fs_close_free return error."
...
failure:
	...
	if (ext2fs_has_feature_journal_needs_recovery(sb)) {
		...
		log_out(ctx, _("skipping journal recovery "
