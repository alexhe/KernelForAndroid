	...
	if (hisifd->index == PRIMARY_PANEL_IDX) {
		if (hisifd_list[EXTERNAL_PANEL_IDX]) {
			if (locked == 0) {
				...
				HISI_FB_ERR("unbalance lock, locked=%d\n", locked);
			}
		}
	}
...
static void hisi_fb_displayeffect_update(struct hisi_fb_data_type *hisifd)
{
	...
	if (!(hisifd->effect_gmp_copy_status & BIT(0)))
	...
	HISI_FB_INFO("[effect]copy_status=%d\n", hisifd->effect_gmp_copy_status);
	hisifd->effect_gmp_copy_status = 0x0;
#else
...
static int hisifb_dirty_region_updt_set(struct fb_info *info, void __user *argp)
{
	...
	if (g_enable_dirty_region_updt
		...
		|| !hisifb_hiace_roi_is_disable(hisifd)
#endif
		)
		&& !hisifd->esd_happened
