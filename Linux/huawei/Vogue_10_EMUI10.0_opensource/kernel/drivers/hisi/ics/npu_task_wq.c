...
static void change_inst_data(unsigned int core_id, unsigned long data, int site)
{
	...
	tmp = (unsigned int)(site/8+1);
	tmp1 = (unsigned int)(site%8);/*lint !e571*/
	if (!tmp1)
...
void npu_boot_inst_init(unsigned int core_id)
{
	...
	if (memset_s(boot_inst, sizeof(task_wq_private[core_id].boot_inst_set.boot_inst), 0, sizeof(task_wq_private[core_id].boot_inst_set.boot_inst)) != EOK) {
...
static int npu_start_wq_task(unsigned int core_id)
{
	...
	if(NPU_TO_START != ioread32((void *)SOC_ICS_START_ADDR((uintptr_t)reg_space->config_reg_virt_addr))) {
		if (kfifo_peek(&task_fifo_ptr->FIFO_TaskElements, &head)) {
			...
			if (memset_s(&task_fifo_ptr->boot_inst_set.boot_inst_tmp[0], sizeof(task_fifo_ptr->boot_inst_set.boot_inst_tmp),0, sizeof(task_fifo_ptr->boot_inst_set.boot_inst_tmp))!= EOK) {
				NPU_ERR("memset failed!");
			}
			if (memcpy_s(&task_fifo_ptr->boot_inst_set.boot_inst_tmp[0], sizeof(task_fifo_ptr->boot_inst_set.boot_inst_tmp), &task_fifo_ptr->boot_inst_set.boot_inst[0], sizeof(task_fifo_ptr->boot_inst_set.boot_inst)) != EOK) {
				NPU_ERR("memcpy failed!");
			}
...
int npu_push_wq_task(const void *arg, unsigned int irq_num)
{
	...
	if(memcpy_s((void *)&task_element, sizeof(NPU_SSWQ_TASK_ELEMENT_S), arg, sizeof(NPU_SSWQ_TASK_ELEMENT_S)) != EOK) {
