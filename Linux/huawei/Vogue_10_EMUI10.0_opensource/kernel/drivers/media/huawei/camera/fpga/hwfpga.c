...
#define SD2HWFPGA(sd) container_of(sd, struct hwfpga_t, subdev)
...
static int hwfpga_v4l2_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (!sd || !fh) {
		HWCAM_CFG_ERR("sd or fh is null");
		return -EINVAL;
	}
...
static struct v4l2_subdev_internal_ops s_hwfpga_subdev_internal_ops = {
...
static long hwfpga_subdev_get_info(struct hwfpga_t *fpga, hwfpga_info_t *info)
{
	...
	if (!fpga || !info) {
		HWCAM_CFG_ERR("fpga or info is null");
		return -EINVAL;
	}
	...
	unsigned int cmd, void *arg)
{
	...
	if (!sd || !arg) {
		HWCAM_CFG_ERR("sd or arg is null");
		return -EINVAL;
	}
...
#define NOTIFY2HWFPGA(i) container_of(i, struct hwfpga_t, notify)
	...
	hwfpga_event_t *fpga_ev)
{
	...
	if (!fpga_ev || !i) {
		HWCAM_CFG_ERR("invalid arguments");
		return;
	}
...
static struct hwfpga_notify_vtbl_t s_notify_hwfpga = {
	.error = hwfpga_notify_error,
};

static struct v4l2_subdev_core_ops s_hwfpga_subdev_core_ops = {
...
static struct v4l2_subdev_ops s_hwfpga_subdev_ops = {
	...
	const struct hwfpga_intf_t *i, struct hwfpga_notify_intf_t **notify)
{
	...
	if (rc < 0) {
		HWCAM_CFG_ERR("isnprintf_s fail");
		kzfree(fpga);
		return -1;
	}
