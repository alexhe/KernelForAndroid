 ...
 * @note       :
********************************************************************************/
int inse_shutdown_swipe_t_nfc_en_func(const void *buf, int para)
{
 ...
 * @note       :
********************************************************************************/
int inse_shutdown_swipe_t_nfc_dis_func(const void *buf, int para)
{
	unsigned int reg_value;
...
static int hisee_pdswipe_flash_open(int flags)
{
	...
	if (1 == g_hisee_partition_byname_find) {
		ret = flash_find_ptn_s(HISEE_IMAGE_PARTITION_NAME, buf, SZ_4K);
		if (0 != ret) {
			pr_err("%s()-line=%d\n", __func__, __LINE__);
		}
	} else {
	...
	(void)memset_s(p_name, sizeof(p_name), 0, sizeof(p_name));
	ret = strncpy_s(p_name, sizeof(p_name), buf, HISEE_BUF_SHOW_LEN);
	if (ret != EOK) {
...
ssize_t hisee_pdswipe_record_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (cnt > 0) {
		ret = memcpy_s((void *)buf, min_t(size_t, PDSWIPE_RECORD_SIZE, PAGE_SIZE),
					(const void *)p_buf, min_t(size_t, PDSWIPE_RECORD_SIZE, PAGE_SIZE));
		if (ret == EOK) {
			pr_err("%s()-line=%d\n", __func__, __LINE__);
		} else {
			pr_err("%s():memcpy err.\n", __func__);
...
ssize_t hisee_pdswipe_exc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (cnt > 0) {
		ret = memcpy_s((void *)buf, min_t(size_t, PDSWIPE_EXC_SIZE, PAGE_SIZE),
						(const void *)p_buf, min_t(size_t, PDSWIPE_EXC_SIZE, PAGE_SIZE));
		if (ret == EOK) {
			pr_err("%s():-line=%d\n", __func__, __LINE__);
		} else {
			pr_err("%s():memcpy err.\n", __func__);
