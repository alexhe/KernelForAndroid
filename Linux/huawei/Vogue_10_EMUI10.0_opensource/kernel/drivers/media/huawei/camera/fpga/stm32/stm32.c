...
#define INTF2STM32(i) container_of(i, struct stm32_t, intf)

static struct stm32_t s_stm32;
...
void stm32_notify_error( uint32_t id)
{
	...
	hwfpga_intf_notify_error(s_stm32.notify, &stm32_ev);
}
...
static struct hwfpga_vtbl_t s_vtbl_stm32 = {
...
static struct stm32_t s_stm32 = {
	.name = "stm32",
	.intf = { .vtbl = &s_vtbl_stm32, },
};

static const struct of_device_id s_stm32_dt_match[] = {
	{
...
static struct platform_driver s_stm32_driver = {
	.driver = {
		.name = "huawei,stm32",
		.owner = THIS_MODULE,
		.of_match_table = s_stm32_dt_match,
	},
};
...
static int32_t stm32_platform_probe(struct platform_device *pdev)
{
	...
	s_pdev = pdev;
	return hwfpga_register(pdev, &s_stm32.intf, &s_stm32.notify);
}
...
static int __init stm32_init_module(void)
{
	...
	cam_notice("%s enter", __func__);
	ret = platform_driver_probe(&s_stm32_driver, stm32_platform_probe);
	return ret;
}

static void __exit stm32_exit_module(void)
{
	platform_driver_unregister(&s_stm32_driver);
	if (s_pdev) {
