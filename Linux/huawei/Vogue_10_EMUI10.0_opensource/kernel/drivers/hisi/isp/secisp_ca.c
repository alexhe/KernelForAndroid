...
static int get_ion_mem_info(int fd, tz_sg_list **sglist, unsigned int * sgl_size, struct device *dev)
{
	...
	*sgl_size = sizeof(tz_sg_list) + nents * sizeof(struct ion_page_info);
	tmp_sgl = (tz_sg_list *)kzalloc(*sgl_size, GFP_KERNEL);
	if (tmp_sgl == NULL) {
	...
	for_each_sg(sgl, sg, tmp_sgl->info_length, i) {
		...
		secisp_print_err("for_each_sg i=%d, addr=0x%llx, npages=%d\n",\
			i, tmp_sgl->page_info[i].phys_addr, tmp_sgl->page_info[i].npages);
	}
...
/*
 * Function name:secisp_loadimg_and_disreset
...
int TEEK_secisp_disreset(int sharefd, unsigned int size)
{
	...
	if(g_secisp_creat_teec == 0){
		secisp_print_err("secisp teec not creat.%d\n", g_secisp_creat_teec);
		return -EPERM;
	}
 ...
 *      @ 0-->success, others-->failed.
 */
int TEEK_secisp_static_mem_map(secisp_mem_info *buffer, unsigned int *iova)
{
	TEEC_Result result;
	TEEC_Operation operation = {0};
	u32 origin = 0;
...
/*
 * Function name:tee_secisp_mem_map_buffer
 * Discription: mem cfg and map
 * Parameters:
 *      @ session: the bridge from unsec world to sec world.
 *      @ buffer:  isp mem info
 ...
 *      @ 0-->success, others-->failed.
 */
int TEEK_secisp_dynamic_mem_map(secisp_mem_info *buffer, unsigned int *iova, struct device * dev)
{
	TEEC_Result result;
	TEEC_Operation operation = {0};
	u32 origin = 0;
	...
	static tz_sg_list * dynamic_sgl = NULL;
	...
	ret = get_ion_mem_info(buffer->sharefd, &dynamic_sgl, &dynamic_sgl_size, dev);
	if (ret < 0) {
 ...
 *      @ 0-->success, others-->failed.
 */
int TEEK_secisp_dynamic_mem_unmap(secisp_mem_info *buffer, struct device * dev)
{
	TEEC_Result result;
	TEEC_Operation operation = {0};
	u32 origin = 0;
...
/*
 * Function name:tee_secisp_probe
...
/*
 * Function name:tee_secisp_remove
