 ...
 * Description: stm32 driver implementation file
 * Author: qianhua qianhua2@huawei.com
 * Create: 2019-02-25
 */
...
static u8 g_reg = 0x00;
static u8 g_reg_value = 0x00;
#endif

static struct stm32_spi_priv_data *spi_drv_data;
 ...
 */
static struct err_map mcu_irq_err_table[] = {
...
static struct err_map mcu_common_err_table[] = {
	// 927011020
	/* undefined abnormal */
			     ...
			     u8 *send, u8 *recieve, unsigned int len)
{
	...
	ret = memcpy_s(recieve, len, &(devdata->rx_buf[0]), len);
	if (ret < 0)
		cam_err("%s memcpy_s fail", __func__);
	return status;
}
...
/*
 * mcu spi write function for normal mode
 * MO: reg<<2 val
 * MI: 0x5A 0x00
 */
					...
					u32 addr, u8 len, u8 *data)
{
	...
	boot_spi_write_reg(devdata, send, 2); /* tw0 bytes */
				...
				const char *file)
{
	...
	memset_s(buff, file_size, 0, file_size);
...
{
	struct stm32_spi_priv_data *drv_data = spi_drv_data;
...
int stm32_spi_enable(void)
{
	...
	/* strobe gpip pull up, it will generate rising to enable mcu detection */
	/* not controlled in mcu */
	/* gpio_direction_output(plat_data->strobe_gpio, 1) */
...
int stm32_spi_disable(void)
{
	...
	/* strobe gpip pull up, it will generate rising to enable mcu detection */
	/* not controlled in mcu */
	/* gpio_direction_output(plat_data->strobe_gpio, 0) */
...
static void mcu_parse_dmd_error(struct work_struct *work)
{
	...
	for (index = 0; index < ARRAY_SIZE(mcu_irq_err_table); index++) {
		if ((err & mcu_irq_err_table[index].err_head) != 0) {
			cam_info("%s: index = %d", __func__, index);
			mcu_dsm_client_notify(drv_data,
				mcu_irq_err_table[index].err_name,
				mcu_irq_err_table[index].err_num);
			find = true;;
		}
	}
	...
	if (find == false) {
		cam_info("%s: index = %d, the err is not matched",
			__func__, index);
		mcu_dsm_client_notify(drv_data,
			mcu_common_err_table[OTHER_ERR].err_name,
			mcu_common_err_table[OTHER_ERR].err_num);
	}
}
...
static struct device_attribute dev_attr_mcu =
...
static int stm32_spi_probe(struct spi_device *spi)
{
	...
	drv_data->tx_buf = (void *)__get_free_pages(GFP_KERNEL | GFP_DMA,
						get_order(2 * PAGE_SIZE));
	drv_data->rx_buf = (void *)__get_free_pages(GFP_KERNEL | GFP_DMA,
						get_order(2 * PAGE_SIZE));
	drv_data->irq_buf = (void *)__get_free_pages(GFP_KERNEL | GFP_DMA,
						get_order(PAGE_SIZE));
	...
	/* strobe gpip pull up, it generate rising to enable mcu detection */
	/* not controlled in mcu */
	/* gpio_direction_output(plat_data->strobe_gpio, 0) */
...
#ifdef MCU_READ_REG_DEBUG
	ret = register_camerafs_attr(&dev_attr_mcu);
	if (ret < 0)
...
static const struct of_device_id stm32_spi_dt_ids[] = {
...
static struct spi_driver stm32_spi_drv = {
	...
	.driver = {
		...
		.owner          = THIS_MODULE,
		.bus            = &spi_bus_type,
		.of_match_table = stm32_spi_dt_ids,
	},
};
module_spi_driver(stm32_spi_drv);
