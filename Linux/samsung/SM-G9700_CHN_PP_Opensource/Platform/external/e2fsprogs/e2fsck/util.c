		      ...
		      io_manager manager)
{
	...
	/* check the LAST-1 block if the partition is smaller than 128M*/
	if (ext2fs_get_device_size2(ctx->filesystem_name, 4096, &num_blocks))
		goto cleanup;

	if (num_blocks > 128*1024*1024/4096)
		goto cleanup;

	superblock = num_blocks - 2;
	io_channel_set_blksize(io, 4096);
	if (io_channel_read_blk64(io, superblock,
				-SUPERBLOCK_SIZE, buf))
		goto cleanup;
#ifdef WORDS_BIGENDIAN
