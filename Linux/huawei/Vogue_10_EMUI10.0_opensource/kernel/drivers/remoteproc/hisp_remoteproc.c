...
int sec_rproc_boot(struct rproc *rproc)
{
	...
	if (ret) {
		pr_err("[%s] Failed : memcpy_s loaded_table from rproc->cached_table, ret.%d\n", __func__, ret);
		goto clean_up;
	}
		...
		int offset, int avail)
{
	...
	ret = snprintf_s(name, sizeof(name), sizeof(name) - 1, "cda%d", rproc->num_cdas);/* unsafe_function_ignore: snprintf */
	if (ret < 0) {
		...
		int offset, int avail)
{
	...
	ret = memset_s(isp_share_para, rsc->len, 0, rsc->len);/* unsafe_function_ignore: memset  */
	if (ret != 0)
...
static void rproc_set_shared_clk_init(void)
{
	...
	ret = memset_s(share_para->clk_value, sizeof(share_para->clk_value), 0, sizeof(share_para->clk_value));/* unsafe_function_ignore: memset  */ /*lint !e838 */
	if (ret != 0)
...
#define PART_ISPFW_SIZE 0x00E00000
	...
	if ((hisi_rproc != NULL) && (hisi_rproc->firmware != NULL)) {
		ret = snprintf_s(pathname, sizeof(pathname), sizeof(pathname)-1, "/system/vendor/firmware/%s", hisi_rproc->firmware);
		if (ret < 0) {
			RPROC_ERR("snprintf_s firmware name fail.%d", ret);
			return ret;
		}
	} else {
		ret = snprintf_s(pathname, sizeof(pathname), sizeof(pathname)-1, "/system/vendor/firmware/isp_fw.elf");
		if (ret < 0) {
...
static int hisp_elf_load_segments(const u8 *data, void* dst, int fw_size)
{
	...
	for (i = 0; i < ehdr->e_phnum; i++, phdr++) {
		...
		if (phdr->p_filesz) {
			ret = memcpy_s(ptr, filesz, elf_data + phdr->p_offset, filesz);
			if (ret != 0) {
		 ...
		 */
		if (memsz > filesz) {
			ret = memset_s(ptr + filesz, memsz - filesz, 0, memsz - filesz);/* unsafe_function_ignore: memset */
			if (ret != 0) {
...
#define MEM_BOOTWARE_SIZE              0xF000
	...
	if (ret) {
		RPROC_ERR("memcpy_s ptr from rproc_dev->isp_bin_vaddr fail, ret.%d \n", ret);
		return ret;
	}
	...
	/* put the segment where the remote processor expects it */
	ret = memcpy_s(ptr, memsz, rproc_dev->isp_bin_vaddr +ISP_TEXT_SIZE, memsz);
	if (ret) {
		RPROC_ERR("memcpy_s ptr from rproc_dev->isp_bin_vaddr fail, ret.%d \n", ret);
		return ret;
	}
...
static struct hisi_rproc_data *hisi_rproc_data_dtget(struct device *pdev)
{
	...
	if (use_default_isp()) {
		...
		ret = memset_s(va, share_size, 0, share_size);/* unsafe_function_ignore: memset *//*lint !e668 */
		if (ret != 0) {
...
void coresight_mem_init(struct device *dev)
{
	...
	/*lint !e110 !e82 !e533*/
	g_cs_para.coresight_addr_da = (unsigned long long)(uintptr_t)dma_addr;  /*lint !e110 !e82 !e533 !e626 */
	g_cs_para.coresight_addr_vir = (unsigned long long)(uintptr_t)addr_vir; /*lint !e110 !e82 !e533 !e626 */
...
static int hisi_rproc_probe(struct platform_device *pdev)
{
	...
	rproc_dev->isp_bin_vaddr = NULL;
	rproc_dev->rsctable_vaddr = NULL;
	ret = memset_s(&rproc_dev->hisi_isp_clk, sizeof(struct hisi_isp_clk_dump_s), 0, sizeof(struct hisi_isp_clk_dump_s));/* unsafe_function_ignore: memset */
	if (ret != 0) {
