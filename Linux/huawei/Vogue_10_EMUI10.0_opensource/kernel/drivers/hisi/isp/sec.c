...
int hisp_mem_type_pa_init(unsigned int etype, unsigned long paddr)
{
	...
	if (!sec_process_use_ca_ta()) {
		...
		pr_info("[%s] type.%d, pa.0x%llx\n", __func__, etype, dev->static_mem[etype].pa);
	}
...
static int hisp_ca_ion_meminit(unsigned int etype)
{
	...
	if((ret = of_property_read_u32_array(np, propname[etype], (unsigned int *)(&dev->ion_mem[etype]), dev->mapping_items)) < 0) {
		pr_info("[%s] propname.%s, of_property_read_u32_array.%d\n", __func__, propname[etype], ret);
		return -EINVAL;
	}
	pr_info("[%s] propname.%s, of_property_read_u32_array.%d.(0x%x, 0x%x, 0x%x, 0x%x, 0x%x)\n",
			__func__, propname[etype], ret, dev->ion_mem[etype].type, dev->ion_mem[etype].da,
			dev->ion_mem[etype].size, dev->ion_mem[etype].prot, dev->ion_mem[etype].sec_flag);
...
static int hisp_ca_static_meminit(unsigned int etype)
{
	...
	if((ret = of_property_read_u32_array(np, propname[etype], (unsigned int *)(&dev->static_mem[etype]), dev->mapping_items)) < 0) {
		pr_info("[%s] propname.%s, of_property_read_u32_array.%d\n",
				__func__, propname[etype], ret);
		return -EINVAL;
	}
	pr_info("[%s] propname.%s, of_property_read_u32_array.%d.(0x%x, 0x%x, 0x%x, 0x%x, 0x%x)\n",
			__func__, propname[etype], ret, dev->static_mem[etype].type, dev->static_mem[etype].da,
			dev->static_mem[etype].size, dev->static_mem[etype].prot, dev->static_mem[etype].sec_flag);
...
static struct phy_csi_info_t * hisp_phy_csi_info_vaddr(void)
{
	...
	if(dev->atfshrd_vaddr != NULL)
		return (struct phy_csi_info_t*)(dev->atfshrd_vaddr + dev->tsmem_offset[ISP_CPU_PARM_OFFSET]);
...
static int set_shared_csi_ctrl_info(u64 info_addr)
{
	...
	ret = memcpy_s((void *)(csi_ctrl_info), (CSI_NUM_MAX + 1) * sizeof(unsigned int),
					(void *)(uintptr_t)info_addr, (CSI_NUM_MAX + 1) * sizeof(unsigned int));
	if (ret != 0) {
		pr_err("[%s] wrong : memcpy_s csi ctrl info fail.%d!\n", __func__, ret);
		return ret;
	}
...
int hisp_phy_csi_connect(struct hisp_phy_info_t * phy_info, unsigned int csi_index)
{
	int ret = 0;
	u64 sharepa = 0;
	u64 info_addr = 0;
	...
	ret = memcpy_s(&phy_csi_info.phy_info, sizeof(struct hisp_phy_info_t), phy_info, sizeof(struct hisp_phy_info_t));
	if (ret != 0) {
		pr_err("[%s] wrong : memcpy_s phy info fail.%d!\n", __func__, ret);
		return ret;
	}
	...
	ret = memcpy_s((void *)(shareva), sizeof(struct phy_csi_info_t), (const void *)(&phy_csi_info), sizeof(struct phy_csi_info_t));
	if (ret != 0) {
		pr_err("[%s] wrong : memcpy_s phy csi info fail.%d!\n", __func__, ret);
		return ret;
	}
...
int hisp_phy_csi_disconnect()
{
	int ret = 0;

	ret = atfisp_phy_csi_disconnect();
	if (ret != 0) {
		pr_err("[%s] atfisp_phy_csi_disconnect fail ret.%d!\n", __func__, ret);
		return ret;
	}
...
static int hisi_atf_getdts(struct platform_device *pdev)
{
	...
	if ((ret = of_property_read_u32(np, "use-ca-ta", &dev->use_ca_ta)) < 0 ) {
		pr_err("Failed : use_ca_ta.%d of_property_read_u32.%d\n", dev->use_ca_ta, ret);
		return -EINVAL;
	}
...
static int sec_mem_pool_destroy(unsigned int type)
{
	...
	if (type == HISP_DYNAMIC_SEC) {
		ret = TEEK_secisp_dynamic_mem_unmap(&atf_dev->ion_mem[type], atf_dev->device);
		if(ret < 0) {
...
static unsigned int sec_mem_pool_setup(unsigned int type, unsigned int pool_num, unsigned int align, unsigned int prot)
{
	struct hisi_mem_pool_s *dev = (struct hisi_mem_pool_s *)&hisi_mem_pool_info;
	struct mem_pool_info_s *pool_info = &dev->isp_pool_info[pool_num];
	...
	ret = memset_s(pool_info, sizeof(struct mem_pool_info_s), 0, sizeof(struct mem_pool_info_s));/*lint !e838 */
	if (ret < 0) {
	...
	pr_debug("[%s] pool_num.%d, size.0x%x, pool_size.0x%x\n", __func__,
				pool_num, size, pool_size);

	atf_dev->ion_mem[type].size = pool_size;
	atf_dev->ion_mem[type].prot = prot;

	if (type == HISP_DYNAMIC_SEC) {
		ret = TEEK_secisp_dynamic_mem_map(&atf_dev->ion_mem[type], &pool_addr, atf_dev->device);
		if(ret < 0) {
		...
		unsigned int dynamic_mem_flag)
{
	...
	memset_s(pool_info, sizeof(struct mem_pool_info_s), 0, sizeof(struct mem_pool_info_s));/*lint !e838 */
	map_size = PAGE_ALIGN(size);/*lint !e50 */
	...
	switch(flag) {
		...
		case MAP_TYPE_DYNAMIC_SEC:
			...
			if (sec_process_use_ca_ta() && use_sec_isp()) {
				...
				if(va == 0){
					pr_err("[%s] Failed : sec_mem_pool_setup failed!\n",__func__);
					return 0;
				}
			}
			break;
...
static unsigned int ipp_sec_memory_setup(int sharefd, unsigned int size, unsigned int prot)
{
	struct hisi_atfisp_s *dev = (struct hisi_atfisp_s *)&atfisp_dev;
...
void hisp_mem_pool_init(void)
{
	...
	memset_s(dev, sizeof(hisi_mem_pool_info), 0x0, sizeof(hisi_mem_pool_info));/*lint !e838 */
...
static int smmu_err_addr_init(void)
{
	...
	/* set smmu err addr pa = bootware addr pa */
		if (dev->shrdmem->a7mapping[A7BOOT].a7pa != 0) {
			dev->sec_smmuerr_addr = dev->shrdmem->a7mapping[A7BOOT].a7pa;
		}
	} else {
		if (dev->ion_mem[HISP_SEC_TEXT].pa != 0) {
			dev->sec_smmuerr_addr = dev->ion_mem[HISP_SEC_TEXT].pa + dev->rsctable_offset + dev->rsctable_size;
		}
	}
...
static void hisp_get_static_mem_pa(unsigned int type)
{
	...
	switch (type) {
		...
		case HISP_SEC_VR1:
		case HISP_SEC_VQ:
			va = get_vring_dma_addr(&dev->static_mem[type].pa, dev->static_mem[type].size, type);
			break;
		case HISP_SEC_SHARE:
...
static unsigned int hisp_sec_page_setup(int sharefd, unsigned int size)
{
	...
	if (size != dev->ion_mem[HISP_SEC_PAGE].size) {
		pr_err("[%s] Failed : wrong size. dev->ion_mem[%u].size.%u. size.%u\n",
			__func__, HISP_SEC_PAGE, dev->ion_mem[HISP_SEC_PAGE].size, size);
		return 0;
	}
	...
	for (i = 0; i < HISP_SEC_STATIC_MAX_TYPE; i ++) {
		...
		ret = TEEK_secisp_static_mem_map(&dev->static_mem[i], &static_iova);
		if (ret < 0) {
...
static unsigned int hisp_sec_boot_setup(unsigned int type, int sharefd, unsigned int size)
{
	struct hisi_atfisp_s *dev = (struct hisi_atfisp_s *)&atfisp_dev;
	unsigned int ion_iova = 0;
	int ret = 0;
...
int hisp_secmem_pa_set(u64 sec_boot_phymem_addr, unsigned int trusted_mem_size)
{
	...
	if (sec_boot_phymem_addr == 0) {
		pr_err("[%s] Failed : sec_boot_phymem_addr.0x%llx\n", __func__, sec_boot_phymem_addr);
		return -EINVAL;
	}
	...
	switch (type) {
		...
		case HISP_ISP_SEC_POOL:
			if (size % SEC_MEM_MAP_ALIGEN != 0) {
				pr_err("[%s] Failed : size is not divisible by 2M. size is 0x%x\n", __func__, size);
				va = 0;
				break;
			}
...
static int secisp_boot_by_ca_ta(void)
{
	...
	if (size == 0) {
		pr_err("[%s] Failed : HISP_SEC_TEXT mem size.%d\n", __func__, size);
		mutex_unlock(&dev->pwrlock);
		return -1;
	}

	if ((ret = TEEK_secisp_disreset(sharefd, size)) < 0) {
		pr_err("[%s] Failed : TEEK_secisp_loadimg_and_disreset.%d.\n", __func__, ret);
		mutex_unlock(&dev->pwrlock);
		return -1;
	}
	dev->ispops->refs_fw++;
...
int hisi_atfisp_probe(struct platform_device *pdev)
{
	...
	ret = memset_s(dev->static_mem, HISP_SEC_STATIC_MAX_TYPE * sizeof(secisp_mem_info), 0,
				HISP_SEC_STATIC_MAX_TYPE * sizeof(secisp_mem_info));
	if (ret < 0)
		pr_err("[%s] Failed : memset_s dev->static_mem.%d.\n", __func__, ret);

	ret = memset_s(dev->ion_mem, (HISP_SEC_MAX_TYPE - 1) * sizeof(secisp_mem_info), 0,
	...
	if(!sec_process_use_ca_ta()) {
		...
		for (index = 0; index < (HISP_SEC_MAX_TYPE - 1); index ++)
			if ((ret = hisp_ca_ion_meminit(index)) < 0)
				pr_err("Failed : index.%d, hisp_ca_ion_meminit.%d\n", index, ret);

		for (index = 0; index < HISP_SEC_STATIC_MAX_TYPE; index ++)
			if ((ret = hisp_ca_static_meminit(index)) < 0)
				pr_err("Failed : index.%d, hisp_ca_static_meminit.%d\n", index, ret);
	...
	if ((dev->domain = iommu_get_domain_for_dev(dev->device)) == NULL) {
