...
static void wldc_get_imax_by_tx_ability(struct wldc_dev_info *di)
{
	s64 tmp;
	int rx_ratio;
	struct tx_capability *tx_cap = wlc_get_tx_cap();
...
static void wldc_select_charge_param(struct wldc_dev_info *di)
{
	...
	if (di->sysfs_data.cp_iin_thermal < cp_iout_low)
		di->sysfs_data.cp_iin_thermal = cp_iout_low;

	rx_max_iout = wlc_get_rx_max_iout();
	if (di->cur_cp_iout_hth > rx_max_iout * di->volt_ratio)
		di->cur_cp_iout_hth = rx_max_iout * di->volt_ratio;
	if (di->cur_cp_iout_hth > di->sysfs_data.cp_iin_thermal)
		di->cur_cp_iout_hth = di->sysfs_data.cp_iin_thermal;
	if ((di->tx_imax > 0) &&
		...
		struct wldc_dev_info *di, int group)
{
	...
	if ((array_len <= 0) ||
		(array_len % WLDC_VOLT_INFO_TOTAL != 0) ||
		(array_len > WLDC_VOLT_LEVEL * WLDC_VOLT_INFO_TOTAL)) {
		hwlog_err("%s: %s invalid\n", __func__, volt_para);
		return -EINVAL;
	}
	...
	for (i = 0; i < array_len; i++) {
		ret = of_property_read_string_index(np, volt_para,
			i, &tmp_str);
		if (ret) {
			hwlog_err("%s: dts(%s) read fail\n",
				__func__, volt_para);
			return -EINVAL;
		}

		ret = kstrtoint(tmp_str, 10, &idata); /* decimal */
		if (ret) {
	...
	struct wldc_dev_info *di)
{
	...
	if ((array_len <= 0) ||
		(array_len % WLDC_BAT_INFO_TOTAL != 0) ||
		(array_len > WLDC_VOLT_GROUP_MAX * WLDC_BAT_INFO_TOTAL)) {
	...
	for (i = 0; i < array_len; i++) {
		ret = of_property_read_string_index(np, "bat_para",
			i, &tmp_str);
		if (ret) {
			hwlog_err("%s: bat_para dts read failed\n", __func__);
			return;
		}
	...
	struct wldc_dev_info *di)
{
	...
	array_len = of_property_count_strings(np, "init_para");
	if ((array_len <= 0) || (array_len % WLDC_INIT_INFO_TOTAL) ||
		(array_len > WLDC_MODE_MAX * WLDC_INIT_INFO_TOTAL)) {
	...
	for (i = 0; i < array_len; i++) {
		...
		if (ret) {
			di->total_dc_mode = 0;
			hwlog_err("%s: read init_para fail\n", __func__);
			goto parse_err;
		}
