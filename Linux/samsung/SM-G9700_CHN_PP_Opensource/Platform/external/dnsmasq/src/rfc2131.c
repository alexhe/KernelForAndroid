...
static void sendDHCPACK(char *type, void *addr, unsigned char *ext_mac,
                int mac_len, char *interface, char *string, u32 xid);
...
static int sendEventToNetdSocket(int,char[][100]);
static int do_monitor(int sock, int stop_after_cmd);
static int do_cmd(int sock, int argc, char argv[][100]);
      ...
      else
	{
	  ...
	  if (hostname) {
              if(strcmp(iface_name,"wlan0")==0 || strcmp(iface_name,"swlan0")==0) {
                  my_syslog(MS_DHCP | LOG_INFO,"call lease_set_hostname_wifi : %s %s",hostname,iface_name);
                  lease_set_hostname_wifi(lease, hostname, hostname_auth);
                  sendDHCPACK("DHCPACK", &mess->yiaddr, emac, emac_len, iface_name, hostname, mess->xid);
              }else {
                 lease_set_hostname(lease, hostname, hostname_auth);
              }
          } else {
              if(strcmp(iface_name,"wlan0")==0 || strcmp(iface_name,"swlan0")==0) {
                  my_syslog(MS_DHCP | LOG_INFO,"hostname is null : %s",iface_name);
                  sendDHCPACK("DHCPACK", &mess->yiaddr, emac, emac_len, iface_name, hostname, mess->xid);
            }
         }
...
{
  struct in_addr a;
  char ptr[10][100]; //Assuming device name max will be 100 charachters
  ...
  print_mac(daemon->namebuff, ext_mac, mac_len);
    ...
    strcpy(ptr[1],"SecSoftap");
    strcpy(ptr[2],"sta_dhcpack");
    strcpy(ptr[3],addr ? inet_ntoa(a):"");
    strcpy(ptr[4],daemon->namebuff);
    strcpy(ptr[5],string ? string : "(null)");
    my_syslog(MS_DHCP | LOG_INFO, "sta_dhcpack");
    sendEventToNetdSocket(6,ptr);
}
...
static int sendEventToNetdSocket(int argc, char argv[][100]) {
    int sock;
    int cmdOffset = 0;   
    int ret;
    ...
    ret=do_cmd(sock, argc-cmdOffset, &(argv[cmdOffset]));
    close(sock);
    return ret;
}
...
static int do_cmd(int sock, int argc, char argv[][100]) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (asprintf(&tmp_final_cmd, format, final_cmd, argv[i],
            ...
            free(final_cmd);
            free(tmp_final_cmd); // Fix for prevent issue
            return res;
        }
        free(final_cmd);
