...
#define TFA9874_INT_IRQ_BITS        (TFA9874_INT_UVDS | \
	TFA9874_INT_OCPR | \
	TFA9874_INT_OTDS | \
	TFA9874_INT_VDDS)
...
enum EN_VIB_BAT_LEVEL {
	...
	EN_VIB_BAT_LEVEL_1, /* -5, 30%, -10, 30%, -15, 50% */
	...
	uint32_t reg_addr)
{
	...
	if (!i2c_ok) {
		...
		ret = vibrator_reg_read(reg_addr, &read_value);
		if (ret != 0)
...
static void vibrator_irq_work(struct work_struct *work)
{
	...
	is_reset = false;
	reset_ok = false;
	ret = vibrator_reg_read(reg_addr, &irq_out_bits);
	if (ret == 0) {
		i2c_ok = true;
