...
static void mar_notify_speed(unsigned int speed)
{
	...
	if (((speed < USB_SPEED_WIRELESS) && (speed > USB_SPEED_UNKNOWN))
		...
		ret = clk_set_rate(_hisi_usb_phy->gt_aclk_usb3otg, g_usb2otg_aclk_freq);
		if (ret) {
			usb_err("Can't aclk rate set\n");
		}
	} else if ((speed == USB_CONNECT_HOST) && (!hisi_dwc3_is_powerdown())) {
		usb_dbg("set USB3OTG_ACLK_FREQ\n");
		ret = clk_set_rate(_hisi_usb_phy->gt_aclk_usb3otg, g_usb3otg_aclk_freq);
		if (ret) {
			usb_err("Can't aclk rate set\n");
		}
	}
...
static void mar_logic_analyzer_trace_print(void)
{
	int times;
	for(times = 0; times < 10; times++) {
		usb_dbg("[misc 28:0x%x]\n", usb3_misc_reg_readl(USB_MISC_REG_LOGIC_ANALYZER_TRACE_L));
		usb_dbg("[misc 2c:0x%x]\n", usb3_misc_reg_readl(USB_MISC_REG_LOGIC_ANALYZER_TRACE_H));
	}
}

static void mar_cmd_tmo_dbg_print(void)
{
	usb_dbg("+\n");
	dwc3_core_logic_analyzer_trace_set(0);
	mar_logic_analyzer_trace_print();
	dwc3_core_logic_analyzer_trace_set(0x17);
	mar_logic_analyzer_trace_print();
	usb_dbg("-\n");
}
...
/*lint -e438 +esym(438,*)*/
/*lint -e529 +esym(529,*)*/
static int mar_usb31_core_enable_u3(struct plat_hisi_usb_phy *usb_phy)
{
	volatile u32 temp;
	int ret = 0;
	int power_down_flag;
...
static int mar_usb31_core_disable_u3(struct plat_hisi_usb_phy *usb_phy)
{
	usb_dbg("+\n");
...
int hisi_usb_combophy_notify(enum phy_change_type type)
{
	...
	if (PHY_MODE_CHANGE_BEGIN == type) {
		ret = mar_usb31_core_enable_u3(_hisi_usb_phy);
	} else if (PHY_MODE_CHANGE_END == type) {
		mar_usb31_core_disable_u3(_hisi_usb_phy);
	} else {
		usb_err("Bad param!\n");
		ret = -EINVAL;
	}
...
static int mar_get_clk_resource(struct device *dev, struct plat_hisi_usb_phy *usb_phy)
{
	/* get abb clk handler */
...
static int mar_get_dts_resource(struct device *dev, struct plat_hisi_usb_phy *usb_phy)
{
	void __iomem *regs;
...
static void __iomem *mar_get_bc_ctrl_reg(void)
{
	...
	return _hisi_usb_phy->sctrl_reg_base + SC_USB20PHY_MISC_CTRL4;
}

static void mar_set_hi_impedance(void)
{
	void __iomem *bc_ctrl2 = mar_get_bc_ctrl_reg();
		...
		unsigned int usb3_phy_tx_vboost_lvl)
{
	...
	usb3_phy_33v_enable(_hisi_usb_phy);
...
static int dwc3_mar_probe(struct platform_device *pdev)
{
	...
	ret = mar_get_dts_resource(&pdev->dev, _hisi_usb_phy);
	if (ret) {
		usb_err("get_dts_resource failed\n");
		goto err_out;
	}

	_hisi_usb_phy->phy.init = mar_usb3phy_init;
	_hisi_usb_phy->phy.shutdown = mar_usb3phy_shutdown;
	_hisi_usb_phy->phy.shared_phy_init = mar_shared_phy_init;
	_hisi_usb_phy->phy.shared_phy_shutdown = mar_shared_phy_shutdown;
	_hisi_usb_phy->phy.set_hi_impedance = mar_set_hi_impedance;
	_hisi_usb_phy->phy.notify_speed = mar_notify_speed;
	_hisi_usb_phy->phy.cmd_tmo_dbg_print = mar_cmd_tmo_dbg_print;
	_hisi_usb_phy->phy.disable_usb3 = mar_disable_usb3;
	_hisi_usb_phy->phy.get_bc_ctrl_reg = mar_get_bc_ctrl_reg;

	ret = hisi_usb_dwc3_register_phy(&_hisi_usb_phy->phy);
	if (ret) {
		usb_err("hisi_usb_dwc3_register_phyfailed\n");
		goto err_out;
	}
