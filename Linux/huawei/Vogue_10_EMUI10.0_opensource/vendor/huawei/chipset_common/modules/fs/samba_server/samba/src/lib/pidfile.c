...
void pidfile_create(const char *program_name)
{
	...
	if (pid != 0) {
		LOGD("ERROR: %s is already running. File %s exists and process id %d is running. ",name, pidFile, (int)pid);
	...
	if (fd == -1) {
		LOGD("ERROR:can't open %s: Error was %s",pidFile,strerror(errno));
	...
	if (fcntl_lock(fd,SMB_F_SETLK,0,1,F_WRLCK)==False) {
		LOGD("ERROR:%s : fcntl lock of file %s failed. Error was %s",name, pidFile, strerror(errno));
	...
	if (write(fd, buf, strlen(buf)) != (ssize_t)strlen(buf)) {
		LOGD("ERROR:can't write to file %s: %s",pidFile, strerror(errno));
