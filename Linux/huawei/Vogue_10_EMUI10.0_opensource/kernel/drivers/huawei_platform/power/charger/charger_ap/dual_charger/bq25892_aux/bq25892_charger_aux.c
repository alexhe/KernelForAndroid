...
static int bq25892_aux_write_mask(u8 reg, u8 mask, u8 shift, u8 value)
{
	...
	return bq25892_aux_write_byte(reg, val);
}
...
#define BQ25892_AUX_SYSFS_FIELD_RW(_name, r, f) \
	BQ25892_AUX_SYSFS_FIELD(_name, r, f, 0640, bq25892_aux_sysfs_store)

#define BQ25892_AUX_SYSFS_FIELD_RO(_name, r, f) \
	BQ25892_AUX_SYSFS_FIELD(_name, r, f, 0440, NULL)
...
{
	int i;
	int limit = ARRAY_SIZE(bq25892_aux_sysfs_field_tbl);
...
static ssize_t bq25892_aux_sysfs_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct bq25892_aux_sysfs_field_info *info = NULL;
	struct bq25892_aux_sysfs_field_info *info2 = NULL;
	int ret;
	u8 v;
...
static int bq25892_aux_dump_register(char *reg_value, int size)
{
	...
	snprintf(buff, BUF_LEN, "%-10d", bq25892_aux_get_ilim());
	strncat(reg_value, buff, strlen(buff));

	for (i = 0; i < BQ25892_AUX_REG_TOTAL_NUM; i++) {
		ret = bq25892_aux_read_byte(i, &reg[i]);
		if (ret)
...
static int bq25892_aux_get_register_head(char *reg_head, int size)
{
	char buff[BUF_LEN] = {0};
	int i;
...
static void bq25892_aux_irq_work(struct work_struct *work)
{
	...
	ret = bq25892_aux_read_byte(BQ25892_AUX_REG_0B, &reg1);
	ret |= bq25892_aux_read_byte(BQ25892_AUX_REG_0C, &reg);
	ret |= bq25892_aux_read_byte(BQ25892_AUX_REG_0C, &reg);
	if (ret)
...
static irqreturn_t bq25892_aux_interrupt(int irq, void *_di)
{
	...
	hwlog_info("bq25892_aux int happened\n");
...
{
	return i2c_add_driver(&bq25892_aux_driver);
}
