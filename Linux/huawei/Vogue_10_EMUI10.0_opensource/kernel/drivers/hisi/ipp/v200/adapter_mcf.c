...
static int mcf_update_cvdr_rd_cfg_tab(msg_req_mcf_request_t* mcf_request, cfg_tab_cvdr_t* cfg_tab, isp_stripe_info_t* stripe_info, isp_stripe_info_t* stripe_info_DS4, unsigned int stripe_index)
{
	...
	mcf_stride = mcf_request->streams[BI_WARP_0_UVC].stride;
        ...
        dataflow_cvdr_rd_cfg_vp(cfg_tab, &cfmt, mcf_stride, CPE_CLK, format);
    }
...
static int mcf_update_cvdr_wr_cfg_tab(msg_req_mcf_request_t* mcf_request, cfg_tab_cvdr_t* cfg_tab, isp_stripe_info_t* stripe_info, isp_stripe_info_t* stripe_info_DS4, unsigned int stripe_index)
{
	...
	mcf_stride = mcf_request->streams[BO_RESULT_Y].stride;
        ...
        dataflow_cvdr_wr_cfg_vp(cfg_tab, &cfmt, mcf_stride, CPE_CLK, format);
    }
...
static int mcf_update_mcf_cfg_tab(msg_req_mcf_request_t* mcf_request, mcf_config_table_t* cfg_tab, unsigned int stripe_width)
{
    ...
    switch(mcf_request->mcf_cfg.mode)
    {
        case MCF_FULL_MODE:
        {
            ...
            cfg_tab->adjust_y.adjust_get_mask.mask_mult = para->mcf_uv_cfg.adjust_get_mask.mask_mult;
            cfg_tab->adjust_y.adjust_get_mask.mask_sub = para->mcf_uv_cfg.adjust_get_mask.mask_sub;

            cfg_tab->adjust_y.adjust_get_ratio.to_use = 1;
            cfg_tab->adjust_y.adjust_get_ratio.ratio_par_up = para->mcf_uv_cfg.adjust_get_ratio.ratio_par_up;
            cfg_tab->adjust_y.adjust_get_ratio.ratio_par_down = para->mcf_uv_cfg.adjust_get_ratio.ratio_par_down;

            cfg_tab->mcf_uv.to_use = 1;
            cfg_tab->mcf_uv.fill_conflict_cfg.to_use = 1;
            cfg_tab->mcf_uv.fill_conflict_cfg.conflict_th = para->uv_fusion.fill_conflict_cfg.conflict_th;
            cfg_tab->mcf_uv.fill_conflict_cfg.luma_th = para->uv_fusion.fill_conflict_cfg.luma_th;
        ...
        case MCF_PARTIAL_MODE:
        {
            ...
            cfg_tab->mcf_uv.to_use = 1;
            cfg_tab->mcf_uv.fill_conflict_cfg.to_use = 0;
...
static int mcf_dump_request(msg_req_mcf_request_t* req)
{
    ...
    for(i = 0; i < 9; i++)
    {
        pr_info("streams[%d].width = %d,height = %d,stride = %d,buf = %d,format = %d\n",
