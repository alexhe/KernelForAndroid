...
{
	if(memcpy_s(&g_task_private->callback_fun, sizeof(NPU_CALLBACK_FUNC_S),  callback_fun_ptr, sizeof(NPU_CALLBACK_FUNC_S)) != EOK) {
...
int npu_push_task(unsigned int* arg, unsigned int irq_num)
{
	...
	if (memset_s(&task, sizeof(NPU_SSWQ_TASK_ELEMENT_S),0, sizeof(NPU_SSWQ_TASK_ELEMENT_S)) != EOK) {
