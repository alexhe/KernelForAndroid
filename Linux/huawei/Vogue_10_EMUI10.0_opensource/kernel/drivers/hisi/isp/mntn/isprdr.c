...
struct noc_err_para_s ispnoc_err[] = {
	[0] = {
		.masterid = SOC_ISP_1_ISP_CORE_0_MID & SOC_MID_MASK,
...
static void rdr_system_err_dump_work(struct work_struct *work)
{
	...
	list_for_each_entry_safe(entry, tmp, &dev->dump_list, node) {
		...
		} else if (ISP_SYSTEM_STATES == entry->modid) {
...
static void rdr_isp_noc_register(void)
{
	...
	pr_info("%s: enter.\n", __func__);
	for (i = 0; i < sizeof(ispnoc_err) / sizeof(struct noc_err_para_s);
			i++) {
		pr_debug("%s: register rdr exception, i = %d, masterid:%d", __func__,
				i, ispnoc_err[i].masterid);
