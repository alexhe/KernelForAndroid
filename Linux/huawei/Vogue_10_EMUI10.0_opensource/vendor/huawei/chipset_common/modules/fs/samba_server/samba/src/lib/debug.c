...
static int check_log_files(void){
	...
	// for bionic , directory = dirname(directory);
	dirname(directory);
	dir = sys_opendir(directory);
	if (!dir) {
		LOGD("[check_log_files]Unable to open directory: [%s].",directory);
	...
	while ((dname = readdirname(dir))) {
		...
		if (log_index < max_log_files) {
			...
			if (stat(pathlist[log_index]->pathname, &statbuf) < 0) {
				LOGD("[check_log_files]Unable to get stat: [%s]",pathlist[log_index]->pathname);
...
BOOL reopen_logs( void )
{
	...
	if (check_log_files() < 0) {
		LOGD("reopen_logs Error check log fiels when reopen logs.");
	...
	if (!new_dbf) {
		log_overflow = True;
		LOGD("Unable to open new log file %s: %s",debugf, strerror(errno));
