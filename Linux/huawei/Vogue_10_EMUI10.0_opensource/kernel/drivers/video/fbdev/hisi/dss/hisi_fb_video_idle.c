...
static void hisifb_video_idle_buffer_alloc(struct hisi_fb_data_type *hisifd, uint32_t req_size)
{
	...
	if (!video_idle_ctrl->buffer_alloced) {
		...
		if (hisi_dss_alloc_cma_buffer(buf_len, (dma_addr_t *)&buf_addr,
			(void **)&video_idle_ctrl->wb_buffer_base)) {
		...
		if (l3_cache_request(&(video_idle_ctrl->request_params))) {
			...
			hisi_dss_free_cma_buffer(buf_len,
				video_idle_ctrl->wb_phys_addr,
				video_idle_ctrl->wb_buffer_base);
...
void hisifb_video_idle_buffer_free(struct hisi_fb_data_type *hisifd)
{
	...
	if (video_idle_ctrl->wb_buffer_base != 0) {
		hisi_dss_free_cma_buffer(video_idle_ctrl->wb_buffer_size,
			video_idle_ctrl->wb_phys_addr,
			video_idle_ctrl->wb_buffer_base);
...
#else //for v510-
static int hisifb_video_idle_l3cache_request(struct hisifb_video_idle_ctrl *video_idle_ctrl)
{
	int ret = 0;
...
#ifdef CONFIG_HISI_L3CACHE_SHARE
	video_idle_ctrl->request_params.id = DSS_IDLE;
	video_idle_ctrl->request_params.request_size = video_idle_ctrl->l3cache_size / 0x100000; // 3M
	video_idle_ctrl->request_params.type = PRIVATE;
...
static void hisifb_video_idle_l3cache_release(struct hisifb_video_idle_ctrl *video_idle_ctrl)
{
	if (video_idle_ctrl->l3cache_size == 0) {
		return;
	}
...
static void hisifb_video_idle_buffer_alloc(struct hisi_fb_data_type *hisifd, uint32_t req_size)
{
	...
	ion_flags = 0;
	handle = ion_alloc(client, buf_len, PAGE_SIZE, ION_HEAP(ION_GRALLOC_HEAP_ID), ion_flags);
	...
	vic->wb_buffer_base = ion_map_kernel(client, handle);
	if (!vic->wb_buffer_base) {
	...
	if (hisifb_ion_phys(client, handle, &(hisifd->pdev->dev), &(buf_addr), &buf_len)) {
		...
		ion_unmap_kernel(hisifd->buffer_client, vic->wb_handle);
		ion_free(client, handle); //lint !e668
		return;
	}
	...
	vic->wb_phys_addr = buf_addr;
	vic->wb_buffer_size = buf_len;
	vic->wb_handle = handle;

	if (hisifb_video_idle_l3cache_request(vic)) {
		ion_unmap_kernel(hisifd->buffer_client, vic->wb_handle);
		ion_free(hisifd->buffer_client, vic->wb_handle);
		vic->wb_handle = NULL;
		vic->wb_buffer_base = 0;
		vic->wb_phys_addr = 0;
		HISI_FB_ERR("hisifb_video_idle_l3cache_request failed.\n");
	...
	if (vic->sc_access) {
		vic->wb_vir_addr = hisi_iommu_idle_display_map(vic->policy_id, vic->wb_phys_addr,
				vic->wb_buffer_size, vic->l3cache_size, vic->lb_size);
		if (!vic->wb_vir_addr) {
			hisifb_video_idle_l3cache_release(vic);
			ion_unmap_kernel(hisifd->buffer_client, vic->wb_handle);
...
void hisifb_video_idle_buffer_free(struct hisi_fb_data_type *hisifd)
{
	...
	if ((hisifd->buffer_client != NULL) && (vic->wb_handle != NULL)) {
		if (vic->sc_access) {
			//lb_release_quota(video_idle_ctrl->policy_id);
			hisi_iommu_idle_display_unmap(vic->wb_vir_addr, vic->wb_buffer_size);
			vic->wb_vir_addr = 0;
		}
		ion_unmap_kernel(hisifd->buffer_client, vic->wb_handle);
		ion_free(hisifd->buffer_client, vic->wb_handle);
