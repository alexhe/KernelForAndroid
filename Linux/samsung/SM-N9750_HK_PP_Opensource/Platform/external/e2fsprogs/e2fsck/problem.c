...
static struct e2fsck_problem problem_table[] = {
	...
	/* Clearing orphan large inode */
	{ PR_0_ORPHAN_CLEAR_LARGE_INODE,
	  N_("%s @o @i %i (uid=%Iu, gid=%Ig, mode=%Im, size=%Is, deleted by %ID)\n"),
	  PROMPT_NONE, 0 },
	...
	{ PR_5_FREE_INODE_COUNT,
	  N_("Total free @is count wrong (%i, counted=%j).\n"),
	...
	{ PR_5_FREE_BLOCK_COUNT,
	  N_("Total free @bs count wrong (%b, counted=%c).\n"),
	  PROMPT_FIX, PR_PREEN_OK | PR_NO_OK | PR_PREEN_NOMSG },
