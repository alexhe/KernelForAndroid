	...
	unsigned long ddr_addr)
{
	...
	ivp_reg_write(ADDR_IVP_CFG_SEC_REG_START_REMAP_ADDR,
		ivp_addr / SIZE_1MB);
	ivp_reg_write(ADDR_IVP_CFG_SEC_REG_REMAP_LENGTH, len);
	ivp_reg_write(ADDR_IVP_CFG_SEC_REG_DDR_REMAP_ADDR,
		(u32)(ddr_addr / SIZE_1MB));
}
...
int ivp_poweron_remap(struct ivp_device *ivp_devp)
{
	...
	ret = ivp_alloc_secbuff(&ivp_devp->ivp_pdev->dev, IVP_SEC_BUFF_SIZE);

	if (ret) {
		ivp_err("ivp alloc secbuff failed!");
		return ret;
	}
	sec_phy_remap_addr = ivp_secmem_ion->sec_phymem_addr;
#else
	ret = ivp_get_secbuff(
		&ivp_devp->ivp_pdev->dev,
	...
	ret = ivpatf_poweron_remap_secmod(
		...
		IVP_SEC_BUFF_SIZE, sec_phy_remap_addr + SIZE_1MB);
	} else {
	...
	struct ivp_device *ivp_devp)
{
	...
	ivp_media1_regulator = devm_regulator_get(
		&pdev->dev,
		IVP_MEDIA_REGULATOR);
	...
	struct ivp_device *ivp_devp)
{
	...
	ret = of_property_read_u32(
		pdev->dev.of_node,
		OF_IVP_MIDDLE_CLK_RATE_NAME, &clk_rate);
	if (ret) {
		ivp_err("get rate failed, ret:%d", ret);
		return -ENOMEM;
	}
	...
	ret = of_property_read_u32(
		pdev->dev.of_node,
		OF_IVP_LOW_CLK_RATE_NAME, &clk_rate);
	if (ret) {
		ivp_err("get rate failed, ret:%d", ret);
		return -ENOMEM;
	}
	...
	ret = of_property_read_u32(
		pdev->dev.of_node,
		OF_IVP_LOW_TEMP_RATE_NAME, &clk_rate);
	if (ret) {
		ivp_err("get rate failed, ret:%d", ret);
		return -ENOMEM;
	}
...
int ivp_change_clk(struct ivp_device *ivp_devp, unsigned int level)
{
	...
	if (ret == 0) {
		ivp_info("set core success to: %ld",
