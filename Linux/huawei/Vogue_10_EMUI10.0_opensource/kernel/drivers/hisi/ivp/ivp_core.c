...
static int ivp_get_validate_section_info(
	...
	struct image_section_header image_sect)
{
	...
	case IMAGE_SECTION_TYPE_DATA: {
		if (true == ddr_flag) {
			ivp_ddr_addr = (ivp_dev.sects[3].acpu_addr << 4) +
				iova - ivp_dev.sects[3].ivp_addr;
...
static long ioctl_clk_level(unsigned long args)
{
	...
	if (copy_from_user(&level, (void *)(uintptr_t)args, sizeof(unsigned int)) != 0) {
		ivp_err("Invalid input param size.");
		return -EINVAL;
	}
