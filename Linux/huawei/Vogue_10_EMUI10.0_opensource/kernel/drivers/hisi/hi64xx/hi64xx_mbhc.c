...
#ifdef CONFIG_HIGH_RESISTANCE_HS_DET
static struct high_res_hs_dev high_res_dev = {
...
static void hi64xx_high_res_hs_init(struct high_res_hs_dev *high_res_hs_dev,
		struct hi64xx_mbhc_priv *priv)
{
	int ret;
	...
	if (!check_high_res_hs_det_support()) {
		pr_err("%s: not support hs res hs det\n", __func__);
		return;
	}
	ret = high_res_dev_register(high_res_hs_dev, priv);
	if (ret != 0) {
		pr_err("%s: device register fail\n", __func__);
		return;
	}
	...
	if ((res_detect_func->hs_res_calibration == NULL) ||
		(res_detect_func->hs_path_enable == NULL) ||
		(res_detect_func->hs_path_disable == NULL)) {
		pr_info("%s: not support hs res calib\n", __func__);
		return;
	}
	res_detect_func->hs_path_enable(priv->codec);
...
#ifdef CONFIG_HIGH_RESISTANCE_HS_DET
	hi64xx_high_res_hs_init(&high_res_dev, priv);
#endif
