...
{
	(void)power_dts_read_u32(np, "acr_rt_support",
		&di->rt_support, ACR_RT_NOT_SUPPORT);
	(void)power_dts_read_u32(np, "acr_rt_threshold",
		&di->rt_threshold, ACR_RT_THRESHOLD_DEFAULT);
	di->rt_threshold *= 1000; /* 1000: mA to uA unit */
	(void)power_dts_read_u32(np, "acr_rt_fmd_min",
		&di->rt_fmd_min, ACR_RT_FMD_MIN_DEFAULT);
	(void)power_dts_read_u32(np, "acr_rt_fmd_max",
