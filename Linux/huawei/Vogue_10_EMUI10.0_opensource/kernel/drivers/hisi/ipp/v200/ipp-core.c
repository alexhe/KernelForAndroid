...
static int hispcpe_map_kernel(struct hispcpe_s *dev, unsigned long args)
{
    ...
    pr_info("[after map kernel]virt_addr = 0x%lx, da.(0x%x, 0x%x)",(unsigned long)dev->virt_addr, iova, dev->daddr);/*lint !e626 */
    I("-\n");
...
static int mcf_process(struct hispcpe_s* dev, unsigned long args)
{
    ...
    mcf_rate_index = mcf_rate_index > MCF_RATE_INDEX_MAX ? 0 : mcf_rate_index ;
