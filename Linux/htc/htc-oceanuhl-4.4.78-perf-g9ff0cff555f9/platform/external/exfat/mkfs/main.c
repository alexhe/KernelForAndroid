...
uint64_t get_boundary_unit(void)
{
	return param.boundary_unit * get_sector_size();
}
...
static int setup_spc_bits(int sector_bits, int user_defined, off_t volume_size)
{
	...
	if ((uint64_t)volume_size < 256ull * 1024 * 1024)
		return MAX(0, 12 - sector_bits);	/* 4 KB */
	if ((uint64_t)volume_size < 32ull * 1024 * 1024 * 1024)
...
int main(int argc, char* argv[])
{
	...
	uint64_t boundary_unit = 0;

	printf("mkexfatfs %s\n", VERSION);

	while ((opt = getopt(argc, argv, "b:i:n:p:s:V")) != -1)
	{
		switch (opt)
		{
		case 'b':
			boundary_unit = strtoull(optarg, NULL, 10);
		case 'i':
