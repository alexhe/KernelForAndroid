	  ...
	  if (!strcmp(iface_name, ML_IFACE))
	    {
              ...
              //my_syslog(MS_DHCP | LOG_INFO, _("Get ncm0 iface_addr %s"), inet_ntoa(iface_addr));
              mask_addr.s_addr = prefixLengthToIpv4Netmask(24);
              prefix_addr.s_addr = iface_addr.s_addr & mask_addr.s_addr;

              memset(cmd, '\0', 1024);  // strncpy() is not filling leftover with '\0'
                            ...
                            struct in_addr netmask, struct in_addr broadcast, void *vparam)
{
  ...
  for (context = daemon->dhcp; context; context = context->next)
	...
	{
	  /* remove it from the current chain if we've it */
          if (context->next == context->current)
            {
              /* unlink this contexts */
  ...
  for (context = daemon->dhcp; context; context = context->next)
    {
      /* The context is still in the current chain */
      if (context->current == context->next)
        {
          strcpy(daemon->dhcp_buff, inet_ntoa(context->start));
          strcpy(daemon->dhcp_buff2, inet_ntoa(context->end));
...
static int runCmd(const char *cmd) {
  ...
  argc = 0;
  memset(buffer, '\0', 1024);  // strncpy() is not filling leftover with '\0'
  strncpy(buffer, cmd, 1023);   // klockwork: 1024-1 to ensure buffer is trailing zero. PCND0012_DNSMASQ_FIX_KLOCKWORK

  while ((tmp = strsep(&next, " ")))
    {
      argv[argc++] = tmp;
      //my_syslog(MS_DHCP | LOG_INFO, _("%s"), argv[argc-1]);
    }
