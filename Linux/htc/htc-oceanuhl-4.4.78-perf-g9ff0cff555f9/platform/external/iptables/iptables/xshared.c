...
int xtables_lock(int wait, struct timeval *wait_interval)
{
	...
	if (wait == -1) {
//PCND0014_IPTABLES_LOG_FOR_XTABLES_LOCK
#if 1
		if (flock(fd, LOCK_EX) == 0){
			ALOGI("[NET] pid:ppid %d:%d: xtables_lock lock (fd=%d)\n",
				getpid(), getppid(), fd);
			return fd;
		}
#else
...
//PCND0014_IPTABLES_LOG_FOR_XTABLES_LOCK
#if 1
		ALOGI("[NET] pid:ppid %d:%d: Can't lock %s: %s\n",
			getpid(), getppid(), XT_LOCK_NAME, strerror(errno));
#endif
...
//PCND0014_IPTABLES_LOG_FOR_XTABLES_LOCK
#if 1
		/* don't need to reduce log when wait interval is more than 1 second */
		if ((i % 10 == 0) || (wait_interval->tv_sec >= 1)) {
			ALOGI("[NET] pid:ppid %d:%d: i=%d: Another app is currently holding the xtables lock; "
				"still %lds %ldus time ahead to have a chance to grab the lock...\n",
				getpid(), getppid(), i, time_left.tv_sec, time_left.tv_usec);
		}
#endif
...
#if 1
	if (lock >= 0)
		ALOGI("[NET] pid:ppid %d:%d: xtables_lock unlock (lock=%d)\n",
				getpid(), getppid(), lock);
#endif
